...
def measure_fhe_inference(estimators, depth, svd, X_test_final, test_df):
    log_time()
...
def measure_plaintext_inference(model, X_test, test_df):
    log_time()
    print("--- Plaintext Prediction Measurement ---")

    print("\n[STEP 1] Using trained plaintext model.")
    
    print("\n[STEP 2] Processing all records...")
    inference_times = []

    for i in range(1000):
        X_single_record_processed = X_test[i:i+1]

        start_time = time.time()
        prediction = model.predict(X_single_record_processed)
        end_time = time.time()
...
if __name__ == "__main__":
    ...
    y_train = train_df['binary_label'].copy()
    ...
    for model_params in models_to_test:
        ...
        print("\nTraining plaintext RandomForestClassifier...")
        plaintext_model = RandomForestClassifier(n_estimators=estimators, max_depth=depth, random_state=42, n_jobs=-1)
        plaintext_model.fit(X_train_final, y_train)
        print("Plaintext model trained.")

        measure_plaintext_inference(plaintext_model, X_test_final, test_df)
        
        measure_fhe_inference(estimators, depth, n_components_svd, X_test_final, test_df)
